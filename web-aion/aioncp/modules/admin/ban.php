...
if(!check($_GET['name'])) {
	...
	if(check($_POST['account_submit'], $_POST['account_name'])) {
		redirect('admin/ban/name/' . $_POST['account_name']);
	}
	echo '<div class="row">';
			...
			echo '<form class="form-horizontal" method="post">';
				...
				echo '<div class="form-group">';
					echo '<div class="col-sm-offset-2 col-sm-10">';
						echo '<button type="submit" name="account_submit" value="ok" class="btn btn-primary">Ban</button>';
					echo '</div>';
				echo '</div>';
			echo '</form>';
	...
	try {
		...
		if($accountData['ip_force'] == 1) throw new Exception('The account is already banned.');
		
		$banAccount = banAccount($_GET['name']);
		if(!$banAccount) throw new Exception('There was an error updating the database.');
		
		message('Account banned!', 'success');
